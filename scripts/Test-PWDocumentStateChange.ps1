...
if (-not $scriptPath) {
    $scriptPath = Join-Path (Split-Path -Parent $PSScriptRoot) 'scripts\Test-PWDocumentStateChange.ps1'
}
if ([System.Threading.Thread]::CurrentThread.GetApartmentState() -eq 'STA') {
    if (-not (Test-Path -LiteralPath $scriptPath)) {
        throw "MTA relaunch: could not resolve script path. Tried: $scriptPath"
    }
    $staMtaHelper = Join-Path (Split-Path -Parent $PSScriptRoot) 'legacy\StaMtaRelaunch.ps1'
