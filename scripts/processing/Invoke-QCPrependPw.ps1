# Invoke-QCPrependPw.ps1
# Purpose: ProjectWise QC prepend - export incoming PDF, merge/overlay into lane history PDF, upload back to PW.
# Production path (Phase 4B). Invoked when qcPrepend.mode is projectWise, pw, or legacyPw.
# Compatibility: legacy/prepend_qc.ps1 forwards here with a deprecation warning.
# Notes:
...
# - If the lane PDF (*-prod/*-chk/*-rev) doesn't exist yet, it runs the same overlay merge + stamp as
#   subsequent prepends, then qpdf keeps page 1 only (overlay compare page — single sheet, matching stamp position).
# - If it exists, it exports both and prepends (with overlay layers when available), stamps, then updates in PW.
...
#   powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\path\scripts\processing\Invoke-QCPrependPw.ps1" -IncomingDocName "input1.pdf"
...
$prependQcRepoRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..\..')).Path
$prependQcLegacyRoot = Join-Path $prependQcRepoRoot 'legacy'
...
if ($QpdfExe -eq "qpdf") {
  $localQpdf = Join-Path $prependQcRepoRoot "tools\qpdf\qpdf.exe"
  $localQpdfBin = Join-Path $prependQcRepoRoot "tools\qpdf\bin\qpdf.exe"
  if (Test-Path $localQpdf) { $QpdfExe = $localQpdf }
  elseif (Test-Path $localQpdfBin) { $QpdfExe = $localQpdfBin }
}

. (Join-Path $prependQcLegacyRoot 'StaMtaRelaunch.ps1')
...
if ([System.Threading.Thread]::CurrentThread.GetApartmentState() -eq 'STA') {
  ...
  if (-not $scriptPath) { $scriptPath = Join-Path $PSScriptRoot 'Invoke-QCPrependPw.ps1' }
...
. (Join-Path $prependQcLegacyRoot 'Logging.ps1')
. (Join-Path $prependQcLegacyRoot 'Resolve-OverlayExe.ps1')

# Default overlay exe: prefer onedir from build_overlay_exe.ps1, else onefile
if (-not $QcOverlayExe) {
  $found = Select-ExistingOverlayExePath $prependQcRepoRoot
  if ($found) {
    $QcOverlayExe = $found
  } else {
    $QcOverlayExe = (Join-Path $prependQcRepoRoot "dist\qc_overlay_prepend\qc_overlay_prepend.exe")
  }
}
...
function Invoke-PrependQcExportPageOne {
  param(
    [Parameter(Mandatory)][string]$SourcePdf,
    [Parameter(Mandatory)][string]$OutPdf
  )
  if (-not (Test-Path -LiteralPath $SourcePdf)) {
    throw "Page-1 extract failed; source PDF not found: $SourcePdf"
  }
  & $QpdfExe --empty --pages $SourcePdf 1-1 -- $OutPdf | Out-Null
  if (-not (Test-Path -LiteralPath $OutPdf)) {
    throw "qpdf failed to extract page 1: $SourcePdf -> $OutPdf"
  }
}
...
if ($createNewHistoryDoc) {
  Write-Log "History document does not exist yet; seeding from incoming for overlay merge (page 1 only on upload)."
...
$localUpload = $localMerged
$localPageOneExtract = $null
if ($createNewHistoryDoc) {
  if (-not $haveQpdf) {
    throw "qpdf is required for first lane PDF create (extract overlay page 1 after merge/stamp)."
  }
  $localPageOneExtract = Join-Path $tempWorkDir ($baseName + "_page1_$stamp.pdf")
  Write-Log "First lane create: keeping page 1 only from merged PDF (overlay compare page + stamp)."
  Invoke-PrependQcExportPageOne -SourcePdf $localMerged -OutPdf $localPageOneExtract
  $localUpload = $localPageOneExtract
}

# Upload / replace history in PW (parameter name varies by pwps_dab version: LocalPath, SourcePath, etc.)
if ($createNewHistoryDoc) {
  Write-Log "Creating history document in PW..."
  if ($PSCmdlet.ShouldProcess("$IncomingFolderPath\$HistoryDocName", "Create history document from overlay page 1")) {
    New-PWDocument -FolderPath $IncomingFolderPath -FilePath $localUpload -DocumentName $HistoryDocName | Out-Null
    ...
    @($localIncoming, $localHistory, $localMerged, $localPageOneExtract, $overlayEphemeralPage1Master) | Where-Object { $_ } | ForEach-Object { Remove-ItemWithRetry $_ }
  } else {
    Write-Log "WhatIf: would create history document from overlay page 1."
  }
} else {
  ...
  if ($PSCmdlet.ShouldProcess($historyDoc.FullPath, "Update document file content from merged PDF")) {
    ...
    $pwUpdateFileParams = @{ $docParamName = $docArg; $fileParamName = $localUpload }
