...
**Folder resolution (Jun 2026):** `pw_parentguid` on document audit rows is the containing folder GUID. The poller resolves it via `Get-PWFoldersByGUIDs` (fallback `Get-PWFoldersHashTableByGuid`), with `pw_folder_cache` and a startup warm-up of `projectWise.watchList.roots` / Sheets subfolders (`Sync-AuditPollerWatchFolderGuidCache`). Configure under `auditPoller.folderGuidCache` in `appsettings.json`.
