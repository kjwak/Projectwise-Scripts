...
def _stamp_markup_fitz_rects(stamp_path: Path) -> list[list[float]]:
    """Stamp markup /Rect values in fitz view space (CropBox-relative), pikepdf annot order."""
    fitz = _get_fitz()
    rects: list[list[float]] = []
    with fitz.open(stamp_path) as sdoc:
        for annot in sdoc[0].annots() or []:
            if annot.type[1] == "Widget":
                continue
            r = fitz.Rect(annot.rect)
            rects.append([float(r.x0), float(r.y0), float(r.x1), float(r.y1)])
    if not rects:
        raise ValueError(f"No stamp markup rects in {stamp_path}")
    return rects
...
def _copy_stamp_widgets_to_page(
    ...
    *,
    ...
    """Copy AcroForm widget annotations using the same block map as markup."""
    import pikepdf
