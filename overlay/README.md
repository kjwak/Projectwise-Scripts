...
Build **once** on a machine that has Python; deploy **only** `dist\qc_overlay_prepend.exe` to PCs that run your automation (for example ProjectWise triggers). Those hosts do not need Python—PowerShell or any scheduler should call the `.exe` with PDF paths.
...
This uses [`qc_overlay_prepend.spec`](../qc_overlay_prepend.spec) (portable paths) and writes `dist\qc_overlay_prepend.exe`.
